...
- All sibling skills' names and descriptions are accessible
...
result = terminal("grep -r '^name:\\|^description:' ~/.hermes/skills/*/SKILL.md --no-filename 2>/dev/null | head -200")
```

Spawn a **single** subagent that evaluates ALL queries at once (batching avoids 20+ spawns):
```
...
[{"query":"...","verdict":"TRIGGER: skill-name"}, {"query":"...","verdict":"NO_TRIGGER"}]
Queries: 1. ... 2. ...
```
...
- Eval set created and reviewed with user
- Before/after accuracy measured
- Description updated (if improved) or reverted (if degraded)

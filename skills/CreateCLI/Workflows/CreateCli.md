...
Before declaring the CLI done, run the **Verify** skill (`Verify/Workflows/RunVerify.md`) on the generated project — build + type-check must pass, `--help` must be comprehensive, and JSON output must pipe cleanly to `jq`. Gate "done" on a READY verdict.

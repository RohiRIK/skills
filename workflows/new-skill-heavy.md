...
chain: "Iterate:RunLenses → Spec → CreateSkill → CreateSkill:TestSkill → Verify → GitHubOps:PullRequest"
---
...
```
Iterate:RunLenses → Spec → CreateSkill → CreateSkill:TestSkill → Verify → GitHubOps:PullRequest
```

## Steps

1. **Iterate:RunLenses** — 2–8 lens passes to surface hidden requirements / edge cases before committing a structure.

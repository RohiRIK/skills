...
chain: "Research(standard|deep) → Spec → Build → Verify → GitHubOps:PullRequest"
---
...
```
Research(standard|deep) → Spec → Build → Verify → GitHubOps:PullRequest
```
...
3. **Spec → Build → Verify → PR** — hand to the build chain.

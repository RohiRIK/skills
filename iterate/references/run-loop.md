...
3. **VERIFY** — call the **Verify** skill (`Verify/references/run-verify.md`) on the target.
...
4. **REFLECT** — call the **Reflect** skill (`Reflect/references/run-reflect.md`) on the pass output. Record the overall score (Reflect writes it to state rather than printing the full card inside a loop).

...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: {{ENV_VAR_NAME}} not found in the project/.env');
      process.exit(1);
    }
    ...
    console.error(`Error: Cannot read the project/.env file`);
    console.error('Make sure {{ENV_VAR_NAME}} is set in the project/.env');
    process.exit(1);
  }
}
```
...
**Reference:** ``Patterns.md`` (Configuration Flags section)
...
For more information, see <output-dir>/{{CLI_NAME}}/README.md
...
See: <output-dir>/{{CLI_NAME}}/README.md
```
...
**Report to user:**
```
✅ CLI Created: <output-dir>/{{CLI_NAME}}/
...
Next steps:
1. Configure: Add {{ENV_VAR_NAME}} to the project/.env
2. Test: ./{{CLI_NAME}}.ts --help
3. Use: ./{{CLI_NAME}}.ts {{EXAMPLE_COMMAND}}

Documentation: <output-dir>/{{CLI_NAME}}/README.md
```
...
**Generated Output:**
```
✅ CLI Created: <output-dir>/notioncli/
...
Next steps:
1. Add NOTION_API_KEY=your_key to the project/.env
...
Do not bake in an output path. Ask the user where to place the generated CLI — default suggestion is the current project directory, or a path they specify. Use `<output-dir>` throughout as that chosen location.

## Quality Gate

Before declaring the CLI done, run the **Verify** skill (`Verify/references/run-verify.md`) on the generated project — build + type-check must pass, `--help` must be comprehensive, and JSON output must pipe cleanly to `jq`. Gate "done" on a READY verdict.

## Execution Log

```bash
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","skill":"CreateCLI","workflow":"CreateCli","status":"ok","duration_s":'$SECONDS'}' \

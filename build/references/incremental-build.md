...
Call the **Verify** skill (`Verify/references/run-verify.md`) on the change. **Gate the commit on `READY`** — if Verify returns NOT READY, do not commit; fix the blocking issues (or, under `/iterate`, capture them to `.agent-state.md` and let the next pass replan) and re-verify.

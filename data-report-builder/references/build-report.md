...
Write a build script importing `ReportBuilder` from `scripts/report-kit.ts`. Copy
`scripts/example.ts` as a complete, runnable starting point.
...
import { ReportBuilder, type Row } from "./scripts/report-kit.ts";
...
The full typed API lives in the header docstring of `scripts/report-kit.ts` — read it
...
```bash
bun scripts/recalc.ts ./Report.xlsx
```

...
```bash
bun ~/.claude/skills/Hygiene/scripts/report.ts
```
...
4. If auto-fixable issues are listed, decide whether to apply:
   - Run `bun ~/.claude/skills/Hygiene/scripts/report.ts --fix` to apply and commit
...
- Auto-fix details → `references/auto-fix.md`
- Issue taxonomy → `references/rules.md`

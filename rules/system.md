...
Drivers (call the primitives):
  Loop         — bounded PLAN→ACT→VERIFY→REFLECT passes; calls Verify + Reflect each pass
...
Existing skills wired in:
  Build  → calls Verify at each commit gate; idempotent so Loop can drive it
...
Loop and multi-pass skills persist progress in a repo-local `.agent-state.md`. Fixed sections: `## Goal`, `## Progress`, `## Dead Ends` (never retry these), `## Iterations`, `## Result`. This bridges context between independent passes/agents. Full schema: `_state/StateFileSchema.md`.
